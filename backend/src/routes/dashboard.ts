...
router.get('/stats', authMiddleware, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  try {
    const [sitesRes, installationsRes, plansRes, dossiersRes] = await Promise.all([
      ...
      supabase.from('plans').select('id', { count: 'exact', head: true }).eq('actif', true).eq('statut', 'valide'),
    ...
    if (role === ROLES.ADMIN_APP || role === ROLES.VIEWER) {
      const [p, c, d, ph] = await Promise.all([
        supabase.from('plans').select('id', { count: 'exact', head: true }).eq('statut', 'en_attente'),
        supabase.from('calques').select('id', { count: 'exact', head: true }).eq('statut', 'en_attente'),
        supabase.from('dossiers').select('id', { count: 'exact', head: true }).eq('statut', 'en_attente'),
      ...
      const [p, c, d, ph] = await Promise.all([
        supabase.from('plans').select('id', { count: 'exact', head: true }).eq('statut', 'en_attente').eq('validateur_id', userId),
        supabase.from('calques').select('id', { count: 'exact', head: true }).eq('statut', 'en_attente').eq('validateur_id', userId),
        supabase.from('dossiers').select('id', { count: 'exact', head: true }).eq('statut', 'en_attente').eq('validateur_id', userId),
        supabase.from('photos').select('id', { count: 'exact', head: true }).eq('statut', 'en_attente').eq('validateur_id', userId),
      ]);
      pending = { plans: p.count ?? 0, calques: c.count ?? 0, dossiers: d.count ?? 0, photos: ph.count ?? 0 };
    } else if (role === ROLES.USER) {
      const [p, c, d, ph] = await Promise.all([
        supabase.from('plans').select('id', { count: 'exact', head: true }).in('statut', ['en_attente', 'a_completer']).eq('validateur_id', userId),
        supabase.from('calques').select('id', { count: 'exact', head: true }).in('statut', ['en_attente', 'a_completer']).eq('validateur_id', userId),
        supabase.from('dossiers').select('id', { count: 'exact', head: true }).in('statut', ['en_attente', 'a_completer']).eq('validateur_id', userId),
...
router.get('/quick-access', authMiddleware, async (_req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const [sitesRes, plansRes, dossiersRes] = await Promise.all([
      ...
      supabase.from('plans').select('id, nom').eq('actif', true).eq('statut', 'valide').order('order', { ascending: true }).limit(50),
      supabase.from('dossiers').select('id, nom').eq('statut', 'valide').order('order', { ascending: true }).limit(50),
    ]);
